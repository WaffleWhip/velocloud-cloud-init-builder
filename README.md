...
1. **Use an auth key** – export `TAILSCALE_KEY=tskey-auth-…` before running the script and it will call `tailscale up --auth-key=…` inside the container.
2. **Use a login link** – leave the key unset; after the installer completes run `pct exec <CTID> -- tailscale up --ssh --accept-routes --qr`. That command prints a short-lived login URL/QR (and the CLI will block until you visit it), so open or scan it to finish the tailnet join manually.
